...
class FingerTable:
    ...
    def __init__(self, node_id, node_addr, m_bits=10):
        ...
        for i in range(0,m_bits):
            ...
            self.ids.append((self.node_id+pow(2, i))%pow(2, self.m_bits))
        pass
    ...
    def find(self, identification):
        ...
        pivot = None
        for i in range(0,len(self.finger)-1):
            low = self.finger[i][0]
            hi = self.finger[i+1][0]
            if(low>hi):
                pivot = self.finger[i]

        if(pivot != None and identification<self.finger[0][0]):
            return pivot[1]
        elif(self.node_id<identification<self.finger[0][0]):
            return self.finger[0][1]
        elif(identification<self.finger[0][0]):
            self.finger[i][-1]
        else:
            for i in range(0,len(self.finger)-1):
                tup1 = self.finger[i]
                tup2 = self.finger[i+1]
                if(tup1[0]<identification<=tup2[0]):
    ...
    def refresh(self):
        ...
        ret = []
        for i in range(0,len(self.finger)):
            ret.append((i+1, (self.node_id+pow(2, i))%pow(2, self.m_bits), self.finger[i][1]))
        return ret
...
class DHTNode(threading.Thread):
    ...
    def get_successor(self, args):
        ...
        id_msg = args["id"]
        from_msg = args["from"]
        if contains(self.identification, self.successor_id, id_msg):
            # Found it! Send response.
            self.send(from_msg,{"method":"SUCCESSOR_REP","args":{
                "req_id":id_msg,
                "successor_id":self.successor_id,
                "successor_addr":self.successor_addr,
            }})
    ...
    def stabilize(self, from_id, addr):
        ...
        update_list = self.finger_table.refresh()
        for tup in update_list:
            if tup[1] == self.finger_table.finger[tup[0]-1][0]:
                continue
            else:
                args= {
                    "id": tup[1],
    ...
    def put(self, key, value, address):
        ...
        #print("\n\nSearching on: ", self.identification)
        #print("FT: ", self.finger_table, "\n\n")
